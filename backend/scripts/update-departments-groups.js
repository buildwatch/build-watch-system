...
async function updateDepartmentsAndGroups() {
  try {
    console.log('Starting to update departments and groups...');

    // Clear existing departments and groups
    console.log('Clearing existing data...');
    await Department.destroy({ where: {} });
    await Group.destroy({ where: {} });

    // Create updated departments
    const departments = [
      {
        id: uuidv4(),
    ...
    console.error('Error updating departments and groups:', error);
    process.exit(1);
  }
}

// Run the update function
updateDepartmentsAndGroups(); 