...
async function setAdminUserIds() {
  try {
    ...
    if (execViewer) {
      if (!execViewer.userId) {
        await execViewer.update({
          userId: 'EXEC-0001'
        });
        console.log('✅ Executive Viewer userId set to: EXEC-0001');
      } else {
        console.log('ℹ️  Executive Viewer already has userId:', execViewer.userId);
      }
    } else {
    ...
    console.log('   Unique User ID: EXEC-0001');
